...
buyer_reason.get(
  ...
  async (req, res) => {
    try {
      const cacheKey = "dropdown_root_cause";
      const cached_data = cache.get(cacheKey);
      if (cached_data) {
        res.status(200).send(cached_data);
      } else {
...
buyer_reason.get("/dropdown/actions", authenticateToken, async (req, res) => {
  try {
    const cacheKey = "dropdown_actions";
    const cached_data = cache.get(cacheKey);
    if (cached_data) {
      res.status(200).send(cached_data);
    } else {
...
buyer_reason.get(
  "/dropdown/transaction_id",
  authenticateToken,
  async (req, res) => {
    try {
      const cacheKey = "dropdown_transaction_id";
      const cached_data = cache.get(cacheKey);
      if (cached_data) {
        res.status(200).send(cached_data);
      } else {
...
// update reason
buyer_reason.put(
  "/buyer/update_reason/:transaction_id",
  authenticateToken,
  async (req, res) => {
    try {
      const sql = await sql_serverConn();
